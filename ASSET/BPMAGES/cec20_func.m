 function [f,gg,hh] = cec20_func(x,prob_k)
...
if(prob_k == 2)
    ...
    h(:,8) = x(:,5).*log(abs(x(:,10)-x(:,7))+1e-8)-x(:,5).*log(400-x(:,8)+1e-8)-x(:,10)+x(:,7)-x(:,8)+400;
...
if(prob_k == 34)
    ...
    if initial_flag == 0
        G = load('FunctionPS1_G.txt');
        B = load('FunctionPS1_B.txt');
        P = load('FunctionPS1_P.txt');
        Q = load('FunctionPS1_Q.txt');
        initial_flag = 1;
    end
    Y = G+1j*B;
...
if(prob_k == 35)
    ...
    if initial_flag == 0
        G = load('FunctionPS2_G.txt');
        B = load('FunctionPS2_B.txt');
        P = load('FunctionPS2_P.txt');
        Q = load('FunctionPS2_Q.txt');
        initial_flag = 1;
    end
    Y = G+1j*B;
    %% voltage initilization
...
if(prob_k == 37)
    ...
    if initial_flag == 0
        G = load('FunctionPS11_G.txt');
        B = load('FunctionPS11_B.txt');
        P = load('FunctionPS11_P.txt');
        Q = load('FunctionPS11_Q.txt');
        initial_flag = 1;
    end
    Y = G+1j*B;
    %% voltage initilization
...
if (prob_k == 40)
   ...
   if initial_flag == 0
        P = load('FunctionPS2_P.txt');
        Q = load('FunctionPS2_Q.txt');
        L = load('FunctionPS14_linedata.txt');
        initial_flag = 1;
   end
    %% voltage initilization
    V = zeros(38,1);
